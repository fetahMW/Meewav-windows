...
export default function RoomLaunchDialog({ closeRef, onClose, initialType, fromProfile = false }: { closeRef: RefObject<HTMLButtonElement | null>; onClose: () => void; initialType?: RoomsHomeRoomType; fromProfile?: boolean }) {
  ...
  const settingsStep = desktopStudio ? 0 : 1;
  ...
  const setValue = (id: string, value: string | number | boolean) => setConfig((current) => ({ ...current, ...(id === "program" ? { setlist: undefined } : {}), values: { ...current.values, [id]: value } }));
  const choose = (next: RoomsHomeRoomType) => { clearBase(); setBaseFormat(8); setType(next); setError(""); setStep(0); setStudioSetup(null); setConfig(defaultRoomLaunch(next)); liveRequestId.current = null; };
  ...
  if (type === "cage") return <div className={`room-launch-cage${desktopStudio ? " is-desktop-sequence" : ""}`}><CageLaunchDialog onBack={() => setType(null)} closeRef={closeRef} onClose={onClose} fromProfile={fromProfile} /></div>;
  return <section className={`room-launch${desktopStudio ? " is-desktop-sequence" : ""}${!type ? " is-room-picker" : ""}${desktopStudio && step === studioStep ? " is-studio-step" : ""}`} role="dialog" aria-modal="true" aria-labelledby="room-launch-title" onKeyDown={(event) => {
    ...
    {!type ? <div className="room-launch__types">{(["loge", "place", "wave", "cage", "classe", "scene"] as const).map((id) => {
      ...
      <nav className="room-launch__steps" aria-label="Étapes de lancement">{(desktopStudio ? ["Préparation", "OBS MiWave", "Lancement"] : ["Identité", "Réglages", "Résumé", "Green Room"]).map((label, i) => <span key={label} aria-current={step === i ? "step" : undefined} className={step === i ? "is-active" : ""}>{i < step ? <Check /> : <b>{i + 1}</b>}{label}</span>)}</nav>
      ...
      <div className={`room-launch__body${step === settingsStep ? " is-preparation" : ""}`}>
        {step === 0 ? <><label>Titre du direct<input autoComplete="off" maxLength={100} value={config.title} onChange={(e) => setConfig({ ...config, title: e.target.value })} placeholder={`Mon rendez-vous dans ${spec.label}`} /></label><label>Présentation<textarea maxLength={500} value={config.description} onChange={(e) => setConfig({ ...config, description: e.target.value })} /></label><label>Accès<select value={config.access} onChange={(e) => setConfig({ ...config, access: e.target.value as RoomLaunchConfiguration["access"] })}><option value="public">Public</option><option value="members">Membres</option><option value="invitation">Sur invitation</option></select></label></> : null}
        ...
        ><div className="launch-recap" aria-label="Réglages préparés">
          {config.roomType === 'wave' && config.baseLoop ? <><strong>{config.baseLoop.fileName}</strong><span>{config.values.bpm} BPM · {config.values.key} · {waveLaunchDuration(config.baseLoop.durationSeconds!)}</span></> : null}
          {config.roomType === 'classe' ? <><strong>24 places élèves · {config.classroom?.pricing === 'paid' ? `${((config.classroom?.priceCents ?? 0) / 100).toLocaleString('fr-FR')} € par place` : 'Classe gratuite'}</strong><span>{config.classroom?.allocation === 'contacts' ? `${config.classroom.students.length} places réservées aux contacts` : config.classroom?.allocation === 'mixed' ? `${config.classroom.students.length} contacts · ${24 - config.classroom.students.length} places Premium libres` : 'Places ouvertes aux membres Premium'}</span></> : null}
          {config.roomType === 'scene' ? <><strong>{config.setlist?.title ?? 'Programme libre'}</strong><span>{String(config.values.program).split('\n').filter(line => line.trim()).length} titres au programme</span></> : null}
        </div></RoomLaunchConfirmation> : <GreenHouse host readyLabel="Ouvrir ma régie" onReady={launch} /> : null}
      ...
      <footer><button disabled={opening} onClick={() => { setError(""); if (step === 0) setType(null); else setStep(step - 1); }}><ArrowLeft />Retour</button>{step < greenStep ? <button className="is-primary" disabled={opening || readingAudio || (step >= settingsStep && type === "wave" && !baseFile)} onClick={async () => {
        const invalid = step === studioStep ? null : step === 0 && !desktopStudio ? validateRoomLaunchIdentity(config) : validateRoomLaunch(config); if (invalid) { setError(invalid); return; }
        setError(""); setStep(step + 1);

      }}>{desktopStudio && step === 0 ? 'Préparer OBS MiWave' : step === summaryStep ? 'Régler mon matériel' : 'Continuer'}<ArrowRight /></button> : null}</footer>
    </>}
  </section>;
}
