...
export default function RoomLaunchDialog({ closeRef, onClose, initialType, fromProfile = false }: { closeRef: RefObject<HTMLButtonElement | null>; onClose: () => void; initialType?: RoomsHomeRoomType; fromProfile?: boolean }) {
  ...
  const choose = (next: RoomsHomeRoomType) => { clearBase(); setBaseFormat(8); setType(next); setError(""); setStep(0); setStudioSetup(null); if (next !== "cage") setConfig(defaultRoomLaunch(next)); };
  ...
  const launch = async () => {
    ...
    if (getDesktopApplicationMode() !== 'demo' && runtime.canPrepareHostRoom && config.roomType === 'place') {
      ...
      try {
        ...
        if (mounted.current) { if (studioSetup) saveRoomProductionSetup(id, studioSetup); onClose(); navigate(`/rooms/place?room=${encodeURIComponent(id)}`); }
      } catch (reason) {
    ...
    {!type ? <div className="room-launch__types">{(["loge", "place", "wave", "cage", "classe", "scene"] as const).map((id) => {
      ...
      return <article key={id} className="room-launch__type-card" data-room-type={id}>
        <button className={`room-launch__choose${initialType === id ? " is-suggested" : ""}`} onClick={() => choose(id)}>
          ...
          <span className="room-launch__room-copy"><strong>{label}</strong><span>{id === "cage" ? "Tournoi, championnat ou Open Mic." : ROOM_LAUNCH_SPECS[id].description}</span></span>
      ...
      <nav className="room-launch__steps" aria-label="Étapes de lancement">{(desktopStudio ? ["Identité", "Configuration", "Studio Meewav", "Lancement"] : ["Identité", "Réglages", "Résumé", "Green Room"]).map((label, i) => <span key={label} aria-current={step === i ? "step" : undefined} className={step === i ? "is-active" : ""}>{i < step ? <Check /> : <b>{i + 1}</b>}{label}</span>)}</nav>
      ...
      <div className="room-launch__body">
        ...
        {step === greenStep ? desktopStudio ? <RoomLaunchConfirmation
          ...
          buttonLabel={config.roomType === 'place' ? 'Créer ma Room LIVE' : 'Ouvrir la préparation DEMO'}
          description={config.roomType === 'place'
        ...
        /> : <GreenHouse host readyLabel="Ouvrir ma régie" onReady={launch} /> : null}
      ...
      }}>{step === summaryStep ? desktopStudio ? 'Vérifications finales' : "Régler mon matériel" : "Continuer"}<ArrowRight /></button> : null}</footer>
    </>}
  </section>;
}
