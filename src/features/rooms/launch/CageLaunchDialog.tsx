...
export default function CageLaunchDialog({ onClose, closeRef, fromProfile = false, onBack }: Props) {
  ...
  const launch = async () => {
    ...
    const validation = validateCageLaunch(configuration);
    if (validation) { setError(validation); return; }
    if (studioBlocked) { setError('Vérifie les sources vidéo et audio dans Studio Meewav, ainsi que ta connexion.'); return; }
  ...
  return <section className={`rooms-home-launch-dialog__panel cage-launch${desktopStudio ? " is-desktop-sequence" : ""}${desktopStudio && step === studioStep ? ' is-studio-step' : ''}`} role="dialog" aria-modal="true" aria-labelledby="rooms-home-launch-title" aria-describedby="rooms-home-launch-description" onKeyDown={(event) => {
    ...
    const buttons = Array.from(event.currentTarget.querySelectorAll<HTMLElement>('button:not(:disabled), input:not(:disabled), select:not(:disabled):not([hidden]), [tabindex="0"]'));
    ...
    <nav className="cage-launch__steps" aria-label="Étapes de lancement">{(desktopStudio ? ["Préparation", "Studio Meewav", "Lancement"] : ["Compétition", "Règlement", "Résumé", "Green Room"]).map((label, index) => <span key={label} aria-current={step === index ? "step" : undefined} className={step === index ? "is-active" : ""}><i>{step > index ? <Check size={12} /> : index + 1}</i>{label}</span>)}</nav>
    <div className="cage-launch__body">
      {step === 0 ? <>
        <label className="cage-launch__field"><span>Configuration</span><MeewavSelect value={configuration.templateId ?? ""} onChange={(event) => {
        ...
        }}><option value="">Créer au lancement</option>{configuration.templateId && !templates.some((item) => item.id === configuration.templateId) ? <option value={configuration.templateId}>Depuis mon profil · {configuration.title}</option> : null}{templates.map((template) => <option key={template.id} value={template.id}>{template.configuration.title}</option>)}</MeewavSelect></label>
        ...
        {desktopStudio ? <label className="cage-launch__field"><span>Ouverture</span><MeewavSelect value={publicRoom ? 'public' : 'private'} onChange={event => { setPublicRoom(event.target.value === 'public'); setStudioSaved(false); }}><option value="public">Ouvrir une Room publique</option><option value="private">Conserver ma préparation en privé</option></MeewavSelect></label> : null}
      </> : null}
      {(desktopStudio ? step === rulesStep : step === 0) ? <>
        <div className="cage-launch__grid"><label className="cage-launch__field"><span>Format</span><MeewavSelect value={configuration.format} onChange={(event) => selectFormat(event.target.value as CageLaunchConfiguration["format"])}>{Object.entries(formatLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</MeewavSelect></label><label className="cage-launch__field"><span>Participants</span><MeewavSelect value={configuration.participantCount} onChange={(event) => patch({ participantCount: Number(event.target.value) })}>{participantCounts.map((count) => <option key={count} value={count}>{count} participant{count > 1 ? "s" : ""}</option>)}</MeewavSelect></label></div>
        <p className="cage-launch__note">{formatDescriptions[configuration.format]}</p>
        <div className="cage-launch__grid">
          <label className="cage-launch__field"><span>Régie</span><MeewavSelect value={configuration.productionTeam ?? "solo"} onChange={event => patch({ productionTeam: event.target.value as "solo" | "regisseur" })}><option value="solo">Je pilote seul</option><option value="regisseur">Avec un régisseur</option></MeewavSelect></label>
        ...
        <label className="cage-launch__field"><span>Sélection du roster</span><MeewavSelect value={configuration.rosterMode} onChange={(event) => patch({ rosterMode: event.target.value as CageLaunchConfiguration["rosterMode"] })}>{Object.entries(rosterLabels).map(([value, label]) => <option key={value} value={value} disabled={value === "prepared" && configuration.rosterProfileIds.length === 0}>{label}</option>)}</MeewavSelect></label>
      ...
      /> : step === rulesStep ? <>
        ...
        </> : <div className="cage-launch__grid"><label className="cage-launch__field"><span>Performances</span><MeewavSelect value={configuration.rules.performanceMode} onChange={(event) => patchRules({ performanceMode: event.target.value as CageLaunchConfiguration["rules"]["performanceMode"] })}><option value="successive">Passages successifs</option><option value="alternating">Performances alternées</option><option value="simultaneous">Duel simultané</option></MeewavSelect></label><label className="cage-launch__field"><span>Manches par rencontre</span><MeewavSelect value={configuration.rules.rounds} onChange={(event) => patchRules({ rounds: Number(event.target.value) })}>{roundCounts.map((count) => <option key={count} value={count}>{count} manche{count > 1 ? "s" : ""}</option>)}</MeewavSelect></label></div>}
        <div className="cage-launch__grid">
          <label className="cage-launch__field"><span>Durée d’un passage</span><MeewavSelect value={configuration.rules.passageDurationSeconds} onChange={(event) => patchRules({ passageDurationSeconds: Number(event.target.value) })}>{passageDurations.map((seconds) => <option key={seconds} value={seconds}>{seconds} secondes</option>)}</MeewavSelect></label>
          {!openMic || (configuration.rules.openMicFeedback && configuration.rules.openMicFeedback !== "none") ? <label className="cage-launch__field"><span>{openMic ? "Durée du retour public" : "Durée du vote"}</span><MeewavSelect value={configuration.rules.votingDurationSeconds} onChange={(event) => patchRules({ votingDurationSeconds: Number(event.target.value) })}>{[30, 45, 60, 90, 120].map((seconds) => <option key={seconds} value={seconds}>{seconds} secondes</option>)}</MeewavSelect></label> : null}
        </div>
        {!openMic ? <><div className="cage-launch__grid"><label className="cage-launch__field"><span>Qui vote ?</span><MeewavSelect value={configuration.rules.votingMode} onChange={(event) => patchRules({ votingMode: event.target.value as CageLaunchConfiguration["rules"]["votingMode"] })}><option value="public">Le public</option><option value="jury">Le jury</option><option value="mixed">Public et jury · 50 / 50</option></MeewavSelect></label><label className="cage-launch__field"><span>Égalité dans une rencontre</span><MeewavSelect value={configuration.rules.tieBreak} onChange={(event) => patchRules({ tieBreak: event.target.value as CageLaunchConfiguration["rules"]["tieBreak"] })}><option value="sudden-death">Manche décisive</option><option value="replay">Rejouer la rencontre</option></MeewavSelect></label></div>{configuration.rules.votingMode !== "public" ? <p className="cage-launch__note">Choisis ensuite 1 à 4 jurés parmi les invités en coulisses, avant le premier match.</p> : null}</> : null}
        <details className="launch-advanced"><summary>Règles avancées <span>Absences, remplacements et places libres</span></summary>
        <div className="cage-launch__grid"><label className="cage-launch__field"><span>Délai de reconnexion</span><MeewavSelect value={configuration.rules.disconnectGraceSeconds} onChange={(event) => patchRules({ disconnectGraceSeconds: Number(event.target.value) })}>{[30, 60, 90, 120, 180].map((seconds) => <option key={seconds} value={seconds}>{seconds} secondes</option>)}</MeewavSelect></label><label className="cage-launch__field"><span>Délai pour un absent</span><MeewavSelect value={configuration.rules.noShowGraceSeconds} onChange={(event) => patchRules({ noShowGraceSeconds: Number(event.target.value) })}>{[30, 60, 90, 120, 180].map((seconds) => <option key={seconds} value={seconds}>{seconds} secondes</option>)}</MeewavSelect></label></div>
        ...
        {studioSaved ? <div role="status"><h3>Ton Studio est prêt</h3><p>La préparation et les sources sont enregistrées sur cet appareil. Aucune Room n’a été publiée.</p></div> : <RoomLaunchConfirmation title={configuration.title} pending={pending} onLaunch={launch} disabled={studioBlocked} buttonLabel={publicRoom ? "Ouvrir La Cage" : "Valider mon Studio"}
          description={!publicRoom ? 'Enregistre le format, les règles et les sources Studio Meewav sans publier de Room.' : preview
          ...
          {publicRoom && !preview ? <ul className="room-launch__preflight" aria-label="Vérifications Studio Meewav">
            <li data-ready={studioReadiness.video}><span>Composition vidéo</span><strong>{studioReadiness.video ? 'Prête' : 'À préparer dans Studio Meewav'}</strong></li>
            <li data-ready={studioReadiness.microphone || studioReadiness.music}><span>Entrée audio</span><strong>{studioReadiness.microphone || studioReadiness.music ? 'Prête' : 'À tester dans Studio Meewav'}</strong></li>
