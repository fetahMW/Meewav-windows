...
export default function CageBattlePanel({ cage, disabled, execute }: { cage: CageState; disabled: boolean; execute: (command: RoomToolsCommand) => Promise<unknown> }) {
  ...
  if (format === "open-mic") {
    ...
    return <div className="room-tool-panel room-cage-regie is-open-mic">
      ...
      <div className="room-cage-regie__context"><span><Radio /><small>FORMAT</small><strong>Open mic</strong></span><span><UserRoundCheck /><small>PASSAGE</small><strong>{liveEntry ? `${liveEntry.order} / ${openMicEntries.length}` : "—"}</strong></span><label><Clock3 /><span><small>DURÉE</small><select aria-label="Durée du passage" value={duration} disabled={disabled} onChange={(eventTarget) => void execute({ type: "cage.battle.duration", seconds: Number(eventTarget.currentTarget.value) as 60 | 90 | 120 | 180 })}><option value={60}>1 min</option><option value={90}>1 min 30</option><option value={120}>2 min</option><option value={180}>3 min</option></select></span></label></div>
  ...
  return <div className="room-tool-panel room-cage-regie is-duel">
    ...
    <div className="room-cage-regie__settings"><label><Clock3 /><span><small>Durée / passage</small><select aria-label="Durée par passage" value={duration} disabled={disabled} onChange={(eventTarget) => void execute({ type: "cage.battle.duration", seconds: Number(eventTarget.currentTarget.value) as 60 | 90 | 120 | 180 })}><option value={60}>1 min</option><option value={90}>1 min 30</option><option value={120}>2 min</option><option value={180}>3 min</option></select></span></label><span><RotateCcw /><span><small>Ordre</small><strong>{round % 2 ? "A puis B" : "B puis A"}</strong></span></span></div>
