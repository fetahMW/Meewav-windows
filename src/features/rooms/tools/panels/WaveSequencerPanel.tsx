  ...
  return <div className="room-tool-panel wave-public-vote wave-vote-inbox">
    ...
    {importOpen ? <div className="wave-vote-import-modal" role="presentation">
      <form role="dialog" aria-modal="true" aria-labelledby="wave-vote-import-title" onSubmit={importCandidate}>
        ...
        <div className="wave-vote-import-modal__fields"><label>Artiste<input value={importArtist} maxLength={60} placeholder="Nom de l’artiste" onChange={(event) => setImportArtist(event.currentTarget.value)} /></label><label>Type<MeewavSelect value={importInstrument} onChange={(event) => setImportInstrument(event.currentTarget.value)}><option>Drums</option><option>Basse</option><option>Mélodie</option><option>Acapella</option><option>Guitare</option><option>Ambiance / FX</option></MeewavSelect></label></div>
