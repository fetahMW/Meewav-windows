...
function EntryEditor({ scene, entry, disabled, onSave, onCancel }: { scene: SceneState; entry?: PerformanceEntry; disabled: boolean; onSave: (draft: EntryDraft) => Promise<void>; onCancel: () => void }) {
  ...
  return <form id={entry ? `scene-program-edit-${entry.id}` : "scene-program-create-form"} className="scene-running-order__editor" onSubmit={submit} aria-label={entry ? `Modifier ${entry.title}` : "Nouveau passage"}>
    ...
    <label className="room-tool-field">Intervenant<MeewavSelect disabled={disabled} value={artistId} onChange={(event) => { const person = scene.people.find((item) => item.id === event.currentTarget.value); setArtistId(event.currentTarget.value); if (person) setArtistName(person.name); setPrompterTextId(""); }}><option value="">Autre intervenant / régie</option>{scene.people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</MeewavSelect></label>
    ...
    <label className="room-tool-field">Type<MeewavSelect disabled={disabled} value={kind} onChange={(event) => { const value = event.currentTarget.value as PerformanceEntry["kind"]; setKind(value); if (value === "Présentation" || value === "Autre") setEvaluationEnabled(false); }}>{KINDS.map((value) => <option key={value}>{value}</option>)}</MeewavSelect></label>
    ...
    <label className="room-tool-field">Texte du prompteur<MeewavSelect disabled={disabled} value={prompterTextId} onChange={(event) => setPrompterTextId(event.currentTarget.value)}><option value="">Aucun texte</option>{scene.prompter.texts.filter((text) => !artistId || text.artistId === artistId || text.id === prompterTextId).map((text) => <option key={text.id} value={text.id}>{text.title}</option>)}</MeewavSelect></label>
