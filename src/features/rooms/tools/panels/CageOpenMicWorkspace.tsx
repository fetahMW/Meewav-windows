...
export function CageOpenMicPublic({ runtime, disabled, isControl, accountId, send }: CageWorkspaceProps) {
  ...
  return <>
    ...
    {!entry ? <p className="cage-workspace__notice">Le vote s’ouvrira après le premier passage.</p> : <section className="cage-workspace__section">
      {performed.length > 1 ? <label>Passage<MeewavSelect value={entry.id} onChange={(event) => setSelectedEntryId(event.target.value)}>{performed.map((item) => <option key={item.id} value={item.id}>Passage {item.order} · {runtime.participants.find((artist) => artist.id === item.participantId)?.person.name}</option>)}</MeewavSelect></label> : null}
