...
export default function ScenePrompterPanel({ scene, role, disabled, execute }: { scene: SceneState; role: RoomActorRole; disabled: boolean; execute: (command: RoomToolsCommand) => Promise<unknown> }) {
  ...
  return <div className="room-tool-panel is-prompter scene-prompter">
    ...
    <div className="scene-prompter__selection">
      <label>Texte à lire<MeewavSelect value={activeText?.id ?? ""} disabled={disabled || !scene.prompter.texts.length} onChange={(event) => void runCommand({ type: "scene.prompter.select", textId: event.currentTarget.value })}>{!scene.prompter.texts.length && <option value="">Aucun texte</option>}{scene.prompter.texts.map((text) => <option key={text.id} value={text.id}>{text.title}</option>)}</MeewavSelect></label>
      <label>Pour l’artiste<MeewavSelect value={activeText?.artistId ?? ""} disabled={disabled || !canEdit || !activeText} onChange={(event) => activeText && void runCommand({ type: "scene.prompter.update", textId: activeText.id, patch: { artistId: event.currentTarget.value } })}><option value="">Choisir un artiste</option>{scene.people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</MeewavSelect></label>
    </div>
    ...
    <section className="scene-prompter__adjustments" aria-label="Réglages de lecture">
      ...
      <details className="scene-prompter__details"><summary>Affichage et décompte<ChevronDown /></summary><div className="scene-prompter__options">
        <label>Avant de commencer<MeewavSelect value={scene.prompter.countdown} disabled={disabled} onChange={(event) => void patch({ countdown: Number(event.currentTarget.value) as 0 | 3 | 5 | 10 })}><option value="0">Départ immédiat</option><option value="3">Décompte de 3 s</option><option value="5">Décompte de 5 s</option><option value="10">Décompte de 10 s</option></MeewavSelect></label>
        <label>Alignement<MeewavSelect value={scene.prompter.alignment} disabled={disabled} onChange={(event) => void patch({ alignment: event.currentTarget.value as SceneState["prompter"]["alignment"] })}><option value="left">Gauche</option><option value="center">Centre</option><option value="right">Droite</option></MeewavSelect></label>
        <label>Interligne<MeewavSelect value={scene.prompter.lineHeight} disabled={disabled} onChange={(event) => void patch({ lineHeight: Number(event.currentTarget.value) })}>{[1.1, 1.3, 1.5, 1.55, 1.7, 2].map((value) => <option key={value} value={value}>{value === 1.55 ? "Standard" : value}</option>)}</MeewavSelect></label>
        <label>Pilotage<MeewavSelect value={scene.prompter.controller} disabled={disabled} onChange={(event) => void patch({ controller: event.currentTarget.value as "artist" | "regie" })}><option value="regie">Régie</option><option value="artist">Artiste</option></MeewavSelect></label>
