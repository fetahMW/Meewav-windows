...
const CollaborationComposer = lazy(() => import("./CollaborationComposer"));
...
function formatFollowerLabel(value: number) {
  const count = Math.max(0, Math.trunc(Number(value) || 0));
  if (count >= 1_000_000) {
    return `${(count / 1_000_000).toLocaleString("fr-FR", { maximumFractionDigits: 1 })}M abonnés`;
  }
  if (count >= 1_000) {
    return `${(count / 1_000).toLocaleString("fr-FR", { maximumFractionDigits: 1 })}K abonnés`;
  }
  return `${count.toLocaleString("fr-FR")} abonné${count > 1 ? "s" : ""}`;
}

function formatMediaDuration(durationMs: number | null) {
  const seconds = Math.max(0, Math.round((durationMs ?? 0) / 1_000));
  const minutes = Math.floor(seconds / 60);
  return `${minutes}:${String(seconds % 60).padStart(2, "0")}`;
}

function formatRoleKey(roleKey: string | null, fallback: string) {
  if (!roleKey?.trim()) return fallback;
  return roleKey
    .replace(/[_-]+/g, " ")
    .replace(/\b\p{L}/gu, (letter) => letter.toLocaleUpperCase("fr-FR"));
}

function mergePublishedMedia(
  baseArtist: PreProfileDemoArtist,
  media: PublishedPreProfileMedia[],
): PreProfileDemoArtist {
  const playableMedia = media.filter((item) => Boolean(item.file_url));
  const videos = playableMedia.filter((item) => item.type === "video" || item.type === "short");
  const audios = playableMedia.filter((item) => item.type === "audio");
  const shorts = videos.map((item) => ({
        id: item.id,
        mediaUrl: item.file_url!,
        thumbnailUrl: item.cover_url ?? baseArtist.portraitUrl,
        duration: formatMediaDuration(item.duration_ms),
        title: item.name,
      }));
  const mappedAudios = audios.map((item, index) => ({
        id: item.id,
        mediaUrl: item.file_url!,
        title: item.name,
        subtitle: item.source_pillar ? `Produit via ${item.source_pillar}` : "Produit sur Meewav",
        duration: formatMediaDuration(item.duration_ms),
        color: index % 2 === 0 ? "purple" as const : "cyan" as const,
      }));
  ...
  return {
    ...
    stats: {
      ...baseArtist.stats,
      shorts: videos.length,
      audios: audios.length,
    },
  };
}

function mergePublicProfile(
  baseArtist: PreProfileDemoArtist,
  profile: PublicPreProfile,
  media: PublishedPreProfileMedia[],
): PreProfileDemoArtist {
  const displayName = profile.display_name?.trim() || profile.username?.trim() || "Profil Meewav";
  const goldenLikesCount = Math.max(0, Number(profile.golden_likes_count) || 0);
  const merged = {
    ...baseArtist,
    id: profile.id,
    name: displayName,
    verified: Boolean(profile.is_verified),
    role: formatRoleKey(profile.primary_role_key, "Créateur Meewav"),
    location: profile.zone_name?.trim() || profile.city?.trim() || "Localisation masquée",
    online: Boolean(profile.is_online),
    followersLabel: formatFollowerLabel(profile.followers_count),
    portraitUrl: profile.profile_image_url?.trim() || profile.avatar_url?.trim() || "/avatars/utilisateur.png",
    portraitFallback: displayName.slice(0, 2).toLocaleUpperCase("fr-FR"),
    gradeLevel: profile.grade,
    grade_level: profile.grade,
    gradeStars: profile.grade,
    grade_stars: profile.grade,
    goldenLikesCount,
    golden_likes_count: goldenLikesCount,
    bio: profile.bio?.trim() || "Aucune bio publique pour le moment.",
    stats: {
      ...baseArtist.stats,
      collabAvailable: Boolean(profile.collab_available),
    },
  };

  return mergePublishedMedia(merged, media);
}

function mergePrivateProfile(
  baseArtist: PreProfileDemoArtist,
  profile: PrivatePreProfile,
  media: PublishedPreProfileMedia[],
): PreProfileDemoArtist {
  const displayName = profile.display_name?.trim() || profile.username?.trim() || "Mon profil Meewav";
  const goldenLikesCount = Math.max(0, Number(profile.golden_likes_count) || 0);
  return mergePublishedMedia({
    ...baseArtist,
    id: profile.id,
    name: displayName,
    verified: Boolean(profile.is_verified),
    role: formatRoleKey(profile.primary_role_key, "Créateur Meewav"),
    location: profile.zone_name?.trim() || profile.city?.trim() || "Localisation non renseignée",
    online: Boolean(profile.is_online),
    followersLabel: formatFollowerLabel(profile.followers_count),
    portraitUrl: profile.profile_image_url?.trim() || profile.avatar_url?.trim() || "/avatars/utilisateur.png",
    portraitFallback: displayName.slice(0, 2).toLocaleUpperCase("fr-FR"),
    ...
    goldenLikesCount,
    golden_likes_count: goldenLikesCount,
    bio: profile.bio?.trim() || "Ajoute une bio depuis ton profil.",
    stats: {
      ...baseArtist.stats,
      collabAvailable: Boolean(profile.collab_available),
    },
...
export function HoverPreProfileContent({
  ...
  const [hydratedArtist, setHydratedArtist] = useState<PreProfileDemoArtist | null>(null);
  const artist = hydratedArtist?.id === seedArtist.id || isOwner
  ...
  const [isCollabComposerOpen, setIsCollabComposerOpen] = useState(false);
  ...
  const seedArtistRef = useRef(seedArtist);
  const interactionLockChangeRef = useRef(onInteractionLockChange);
  const collaborationIdempotencyKeyRef = useRef<string | null>(null);
  seedArtistRef.current = seedArtist;
  interactionLockChangeRef.current = onInteractionLockChange;

  useEffect(() => {
    let cancelled = false;
    const seed = seedArtistRef.current;
    const profileId = seed.id;

    setHydratedArtist(null);
    setGoldenLikesCount(Math.max(0, seed.golden_likes_count ?? seed.goldenLikesCount ?? 0));
    setGoldenLikeGivenToday(false);

    if (demoFollow || (!isOwner && !isCanonicalProfileId(profileId))) return () => { cancelled = true; };

    if (!isOwner) {
      void trackPreProfileAnalytics(profileId, "globe_profile_open", {
    ...
    const hydrate = async () => {
      if (isOwner) {
        const [profileResult, mediaResult] = await Promise.allSettled([
          getMyPrivatePreProfile(),
          isCanonicalProfileId(profileId) ? getPublishedPreProfileMedia(profileId) : Promise.resolve([]),
        ]);
        if (cancelled || profileResult.status !== "fulfilled" || !profileResult.value) return;
        const media = mediaResult.status === "fulfilled" ? mediaResult.value : [];
        setHydratedArtist(mergePrivateProfile(seed, profileResult.value, media));
        return;
      }

      const [profileResult, mediaResult, followResult, goldenResult] = await Promise.allSettled([
        getPublicPreProfile(profileId),
        getPublishedPreProfileMedia(profileId),
        isLocalAuthPreviewEnabled()
          ? Promise.resolve({
              authenticated: false,
              viewerProfileId: null,
              following: getLocalPreviewFollowState(profileId),
            })
          : getFollowState(profileId),
        getPreProfileGoldenLikeState(profileId),
      ]);
      if (cancelled) return;

      if (profileResult.status === "fulfilled" && profileResult.value) {
        const media = mediaResult.status === "fulfilled" ? mediaResult.value : [];
        const nextArtist = mergePublicProfile(seed, profileResult.value, media);
        setHydratedArtist(nextArtist);
        setGoldenLikesCount(nextArtist.golden_likes_count ?? 0);
      }
      if (followResult.status === "fulfilled") {
        setIsFollowing(followResult.value.following);
      }
      if (goldenResult.status === "fulfilled" && goldenResult.value.ok) {
        setGoldenLikesCount(Math.max(0, goldenResult.value.goldenLikesCount ?? 0));
        setGoldenLikeGivenToday(goldenResult.value.givenToThisArtistToday);
      }
    };

    void hydrate();
    return () => { cancelled = true; };
  }, [isOwner, seedArtist.id, demoFollow]);
  ...
  useEffect(() => {
    ...
    setIsFollowing(
      !isOwner && demoFollow
        ? demoFollowedProfiles.has(seedArtist.id)
        : !isOwner && isLocalAuthPreviewEnabled()
        ? getLocalPreviewFollowState(seedArtist.id)
        : false,
    );
    ...
    collaborationIdempotencyKeyRef.current = null;
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      if (isLocalAuthPreviewEnabled()) {
        const following = setLocalPreviewFollowState(artist.id, next);
        setIsFollowing(following);
        onFollow?.(artist.id);
        return;
      }
      const persisted = await setFollowState(artist.id, next);
      setIsFollowing(persisted.following);
      onFollow?.(artist.id);
    } catch {
  ...
  const handleShortSelect = (shortId: string) => {
    ...
    if (!isOwner) {
      void trackPreProfileAnalytics(artist.id, "media_play", {
        media_id: shortId,
        media_type: "video",
        surface: "globe_preprofile",
      }).catch(() => undefined);
  ...
  const handleCollabRequest = () => {
    ...
    if (isOwner) {
      if (onCollabRequest) {
        ...
        navigate("/messages?space=collabs&source=globe");
      }
      return;
    }

    setPlayingMedia(null);
    collaborationIdempotencyKeyRef.current = createClientIdempotencyKey();
    void trackPreProfileAnalytics(artist.id, "message_composer_open", {
      composer: "collaboration",
      surface: "globe_preprofile",
    }).catch(() => undefined);
    interactionLockChangeRef.current?.(true);
    setIsCollabComposerOpen(true);
  };

  const closeCollabComposer = useCallback(() => {
    interactionLockChangeRef.current?.(false);
    setIsCollabComposerOpen(false);
    collaborationIdempotencyKeyRef.current = null;
  }, []);
  ...
  const handleContact = () => {
    ...
    if (onContact) {
      ...
      navigate(createGlobeMessagingPath(
        artist.id,
        "message",
        hasRealRecipientProfile ? "real" : "demo",
      ));
    }
    window.dispatchEvent(new CustomEvent("meewav:contact-profile", {
      detail: {
        artistId: artist.id,
  ...
  const allowDemoMedia = !isCanonicalProfileId(artist.id);
    ...
    >
          ...
          <div className="mw-preprofile__meta">
            ...
            <div className="mw-preprofile__golden-like-inline">
              <GoldenLikeCounter
                ...
                interactive={!isOwner && isCanonicalProfileId(artist.id)}
                lockedOpen={!isOwner && isCanonicalProfileId(artist.id)}
                onCountChange={(count) => {
      ...
      <footer className="mw-preprofile__footer" aria-label="Actions de contact">
        <button
          ...
          aria-controls={!isOwner ? "mw-collaboration-composer" : undefined}
        ...
        {!isOwner && isCollabComposerOpen && (
          <div className="mw-collaboration-composer-shell">
            <Suspense
              fallback={(
                <div className="mw-collaboration-composer__loading" role="status">
                  <HeartHandshake size={20} strokeWidth={2.2} aria-hidden="true" />
                  <span>Préparation du composer…</span>
                </div>
              )}
            >
              <CollaborationComposer
                ...
                onSubmit={async (draft) => {
                  ...
                  if (hasRealRecipientProfile) {
                    const requestIdempotencyKey = collaborationIdempotencyKeyRef.current
                      ?? createClientIdempotencyKey();
                    collaborationIdempotencyKeyRef.current = requestIdempotencyKey;
                    ...
                    persistedRequestId = result.requestId;
                  } else {
                    submitGlobeCollaborationRequest({
                      senderProfileId,
                      recipientProfileId: draft.recipientProfileId,
                      recipientName: artist.name,
                      recipientRole: artist.role,
                      recipientAvatar: artist.portraitUrl,
                      recipientGradeLevel: artistGradeLevel ?? undefined,
                      message: draft.message,
                      attachments: draft.attachments,
                    });
                  }
                  if (onCollabRequest) {
                    onCollabRequest(draft.recipientProfileId);
                  } else {
                    navigate(createGlobeMessagingPath(
                      draft.recipientProfileId,
                      "collaboration",
                      hasRealRecipientProfile ? "real" : "demo",
                      persistedRequestId,
                    ));
