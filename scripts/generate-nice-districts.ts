...
const OUTPUT_PATH = path.join(ROOT_DIR, "public", "map", "nice-quartiers.geojson");

const NICE_COMMUNE_CODE = "06088";
const EXPECTED_NICE_IRIS_COUNT = 146;
const EXPECTED_NICE_TERRITORY_COUNT = 6;
const IRIS_WFS_URL = "https://data.geopf.fr/wfs/ows";
const IRIS_DATASET_URL = "https://geoservices.ign.fr/contoursiris";
const TERRITORIES_DATASET_URL = "https://opendata.nicecotedazur.org/data/dataset/limite-des-territoires-de-la-ville-de-nice";
const TERRITORIES_GEOJSON_URL = "https://opendata.nicecotedazur.org/data/storage/f/2021-08-18T08%3A52%3A55.556Z/ig-territoire.geojson";
const NICE_BBOX = [7.17, 43.63, 7.34, 43.78] as const;
...
async function fetchJson(url: URL | string) {
  const response = await fetch(url, {
    headers: {
      ...
      "user-agent": "Meewav-Web Nice districts generator/1.0",
    },
  });
...
async function fetchIrisCollection() {
  ...
  url.searchParams.set("BBOX", `${NICE_BBOX.join(",")},EPSG:4326`);
  url.searchParams.set("SRSNAME", "EPSG:4326");
  return fetchJson(url);
}

function getTerritoryForIris(
  irisFeature: GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>,
  territories: Array<GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon, Record<string, unknown>>>,
) {
  const candidates = [geoCentroid(irisFeature) as [number, number], getInteriorLabelPoint(irisFeature)];
  for (const candidate of candidates) {
    const territory = territories.find((feature) => isInside(feature, candidate));
    if (territory) return territory;
  }
  throw new Error(`Unable to assign IRIS ${String(irisFeature.properties?.code_iris ?? "unknown")} to a Nice territory`);
}
...
function normalizeIris(
  ...
  if (communeCode !== NICE_COMMUNE_CODE || !irisCode || !localIrisCode || !label) {
    throw new Error(`Invalid Nice IRIS: ${JSON.stringify(properties)}`);
  }
  assertPolygonFeature(feature, label);

  const territory = getTerritoryForIris(feature, territories);
  const parentCode = String(territory.properties.IDENT ?? "").trim();
  const parentLabel = String(territory.properties.SECTEUR ?? "").trim();
  if (!parentCode || !parentLabel) throw new Error(`Invalid Nice territory for ${label}`);

  const zoneId = `nice_${irisCode}_${slugify(label)}`;
  ...
  return {
    ...
    properties: {
      ...
      colorIndex,
      parentZoneId: `nice_parent_${parentCode}_${slugify(parentLabel)}`,
      parentCode,
      parentLabel,
      territoryType: "iris",
...
async function main() {
  const [irisPayload, territoryPayload] = await Promise.all([
    fetchIrisCollection(),
    fetchJson(TERRITORIES_GEOJSON_URL),
  ]);

  const territoryFeatures = (Array.isArray(territoryPayload?.features) ? territoryPayload.features : []) as GeoJSON.Feature[];
  if (territoryFeatures.length !== EXPECTED_NICE_TERRITORY_COUNT) {
    throw new Error(`Expected ${EXPECTED_NICE_TERRITORY_COUNT} Nice territories, received ${territoryFeatures.length}`);
  }
  for (const feature of territoryFeatures) assertPolygonFeature(feature, String(feature.properties?.SECTEUR ?? "territory"));
  const territories = territoryFeatures as Array<GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon, Record<string, unknown>>>;

  const irisFeatures = (Array.isArray(irisPayload?.features) ? irisPayload.features : [])
    .filter((feature: GeoJSON.Feature) => String(feature.properties?.code_insee ?? "") === NICE_COMMUNE_CODE) as GeoJSON.Feature[];
  if (irisFeatures.length !== EXPECTED_NICE_IRIS_COUNT) {
    throw new Error(`Expected ${EXPECTED_NICE_IRIS_COUNT} Nice IRIS, received ${irisFeatures.length}`);
  }

  irisFeatures.sort((left, right) => String(left.properties?.code_iris).localeCompare(String(right.properties?.code_iris)));
  const features = irisFeatures.map((feature, index) => normalizeIris(feature, territories, index % 8));
  const zoneIds = new Set(features.map((feature) => feature.properties.zoneId));
  if (zoneIds.size !== features.length) throw new Error("Duplicate Nice IRIS zone IDs");

  const parentCodes = new Set(features.map((feature) => feature.properties.parentCode));
  if (parentCodes.size !== EXPECTED_NICE_TERRITORY_COUNT) {
    throw new Error(`Expected all ${EXPECTED_NICE_TERRITORY_COUNT} Nice territories to own IRIS, received ${parentCodes.size}`);
  }
  ...
  const collection = {
    ...
    metadata: {
      ...
      source: "IGN / INSEE Contours IRIS and Ville de Nice territories",
      datasetId: "STATISTICALUNITS.IRIS:contours_iris / 09132324-6787-43ee-ad44-4ca36fab06b1",
      datasetUrl: IRIS_DATASET_URL,
      parentDatasetUrl: TERRITORIES_DATASET_URL,
      parentSourceYear: "2021",
  ...
  console.log(`Generated ${features.length} official Nice IRIS polygons at ${OUTPUT_PATH}`);
}
