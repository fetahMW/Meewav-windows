-- Collaboration requests can originate from every authenticated MeeWav
-- surface. The source is attribution metadata only; recipient availability,
-- rate limits, idempotency and row ownership remain server-enforced.
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended('profile-collaboration:' || v_sender_profile_id::text, 0));
  ...
  if v_request.id is not null then
    ...
    if (
      select count(*) from public.collaboration_requests r
      where r.sender_profile_id = v_sender_profile_id
    ...
    insert into public.collaboration_requests (
      sender_profile_id, recipient_profile_id, message, status, idempotency_key, source
    ) values (
      v_sender_profile_id, p_recipient_profile_id, v_message, 'pending', v_idempotency_key, v_source
    ) returning * into v_request;

    insert into public.analytics_events (
      actor_profile_id, subject_profile_id, source_pillar, event_name,
      source_event_id, idempotency_key, trust_level, properties, occurred_at
    ) values (
      v_sender_profile_id, p_recipient_profile_id, v_source,
      'collaboration_request_created', v_request.id::text,
      'collaboration:' || v_request.id::text, 'server',
      jsonb_build_object('status', v_request.status), v_request.created_at
...
comment on function public.request_profile_collaboration(uuid, text, text, text) is
  'Authenticated idempotent collaboration creation with cross-feature source attribution.';
